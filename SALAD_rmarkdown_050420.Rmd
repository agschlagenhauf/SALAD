...
data_strials <- read_csv2(sing_trial_file,na = c("999", "NA"))
...
data_strials$choiceanova <- data_strials$Choice_t
data_strials$choiceanova[data_strials$Choice_t == 0] <- -1

# our version of winstay
data_strials$winstayanova <- data_strials$choiceanova*data_strials$winstay_t1
data_strials$winstayanova[data_strials$winstayanova == -1] <- 0

# our version of loseshift
data_strials$loseshiftanova <- data_strials$choiceanova*data_strials$loseshift_t1
data_strials$loseshiftanova[data_strials$loseshiftanova == -1] <- 0

# use single trial dataset to predict correct (Choice_t is single-trial equivalent to p_correct)

mod1<-glmer(Choice_t~Group*Cond*volat+(1|sub_idx), data=data_strials,family=binomial)

mod2<-glmer(winstayanova~Group*Cond*volat+(1|sub_idx), data=data_strials,family=binomial)

mod3<-glmer(loseshiftanova~Group*Cond*volat+(1|sub_idx), data=data_strials,family=binomial)

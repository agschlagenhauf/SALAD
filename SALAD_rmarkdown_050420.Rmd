...
if (!require(ez)) install.packages("broom")
if (!require(strex)) install.packages("strex")
if (!require(nlme)) install.packages("nlme")
if (!require(lattice)) install.packages("lattice")
if (!require(tinytex)) install.packages('tinytex')
if (!require(tinytex)) install.packages('purrr')
if (!require(knitr)) install.packages("knitr")
if (!require(kableExtra)) install.packages('kableExtra')
if (!require(stringr)) install.packages('stringr')
if (!require(stringr)) install.packages('hypr')
if (!require(stringr)) install.packages('schoRsch')
if (!require(stringr)) install.packages('emmeans')
if (!require(stringr)) install.packages('psycho')
...
res.HC.lswitch <- ezANOVA(longdat.HC.lswitch, p_lswitch, id, within = .(volat,cond), detailed = TRUE, type = 3)
...
f3 <- ggplot(data=longdat.correct, aes(y=p_correct)) +
  
   #Add geom_() objects
   geom_point(aes(x=jvolat2), color = "magenta", size = 1.5) +
   geom_line(aes(x=jvolat2, group=sub_id), color = 'lightgray') +
...
f4 <- ggplot(data=longdat.correct, aes(y=p_correct)) +
   ...
   labs(title = 'Correct responses decreased in reversal phase', 
           subtitle = 'Control vs. stress condition by phase',
           caption = 'Datapoints are not jittered') + 
...
f5 <- ggplot(data = longdat.correct, aes(y = p_correct)) +
  
   #Add geom_() objects
   geom_point(data = longdat.correct %>% filter(cond2 =="0"), aes(x=jvolat2), color = 'dodgerblue', size = 1.5, 
              alpha = .6) +
   geom_point(data = longdat.correct %>% filter(cond2 =="1"), aes(x=jvolat2), color = 'darkorange', size = 1.5, 
              alpha = .6) +
   geom_line(aes(x = jvolat2, group = sub_id), color = 'lightgray', alpha = .3) +
  
   geom_half_boxplot(
     data = longdat.correct %>% filter(cond2=="0"), aes(x=jvolat2, y = p_correct), position = position_nudge(x = -3), 
     side = "r",outlier.shape = NA, center = TRUE, errorbar.draw = FALSE, width = .2, 
     fill = 'dodgerblue') +
   
   geom_half_boxplot(
     data = longdat.correct %>% filter(cond2=="1"), aes(x=jvolat2, y = p_correct), position = position_nudge(x = 2), 
     side = "r",outlier.shape = NA, center = TRUE, errorbar.draw = FALSE, width = .2, 
     fill = 'darkorange') +
   ...
   #Define additional settings
   scale_x_continuous(breaks=c(1,2,3,4,5,6), labels=c("Control_pre","Control_rev","Control_post","Stress_pre", "Stress_rev", "Stress_post"), limits=c(-3, 10)) +
...
source("import_singletrial_data.R")
...
# subject as random intercept model
mod1a.HC.correct <- glmer(Correct~Cond*volat+(1|sub_idx), data=data_new_HC,family=binomial, nAGQ = 0)

mod1a.HC.w_stay <- glmer(w_stay~Cond*volat+(1|sub_idx), data=data_new_HC,family=binomial, nAGQ = 0)

mod1a.HC.l_switch <- glmer(w_stay~Cond*volat+(1|sub_idx), data=data_new_HC,family=binomial, nAGQ = 0)
...
mod0.correct <- glmer(Correct~(1|sub_idx), data=data_new, family=binomial, nAGQ = 0)
mod0a.correct <- glmer(Correct~(1|sub_idx)+school_yrs+age, data=data_new,family=binomial, nAGQ = 0)
mod1.correct <- glmer(Correct~Group+Cond+volat+(1|sub_idx), data=data_new,family=binomial, nAGQ = 0)
mod2.correct <- glmer(Correct~Group*Cond+Group*volat+Cond*volat+(1|sub_idx), data=data_new,family=binomial, nAGQ = 0)
mod3.correct <- glmer(Correct~Group*Cond*volat+(1|sub_idx), data=data_new,family=binomial, nAGQ = 0)

# model comparison of Correct models
anova(mod0.correct, mod0a.correct, mod1.correct,mod2.correct,mod3.correct)
...
mod0.wstay <- glmer(w_stay~(1|sub_idx), data=data_new, family=binomial, nAGQ = 0)
mod0a.wstay <- glmer(w_stay~(1|sub_idx)+school_yrs+age, data=data_new,family=binomial, nAGQ = 0)
mod1.wstay <- glmer(w_stay~Group+Cond+volat+(1|sub_idx), data=data_new,family=binomial, nAGQ = 0)
mod2.wstay <- glmer(w_stay~Group*Cond+Group*volat+Cond*volat+(1|sub_idx), data=data_new,family=binomial, nAGQ = 0)
mod3.wstay <- glmer(w_stay~Group*Cond*volat+(1|sub_idx), data=data_new,family=binomial, nAGQ = 0)

# model comparison of w_stay models
anova(mod0.wstay,mod0a.wstay,mod1.wstay,mod2.wstay,mod3.wstay)
...
mod0.lswitch <- glmer(l_switch~(1|sub_idx), data=data_new, family=binomial, nAGQ = 0)
#mod0a.lswitch <- glmer(l_switch~(1|sub_idx)+age, data=data_new,family=binomial, nAGQ = 0)
mod0b.lswitch <- glmer(l_switch~(1|sub_idx)+IQ_WST, data=data_new,family=binomial, nAGQ = 0)
mod0c.lswitch <- glmer(l_switch~(1|sub_idx)+school_yrs+age, data=data_new,family=binomial, nAGQ = 0)
mod1.lswitch <- glmer(l_switch~Group+Cond+volat+(1|sub_idx), data=data_new,family=binomial, nAGQ = 0)
mod2.lswitch <- glmer(l_switch~Group*Cond+Group*volat+Cond*volat+(1|sub_idx), data=data_new,family=binomial, nAGQ = 0)
mod3.lswitch <- glmer(l_switch~Group*Cond*volat+(1|sub_idx), data=data_new,family=binomial, nAGQ = 0)

# model comparison of lswitch models
anova(mod0.lswitch,mod0b.lswitch,mod0c.lswitch,mod1.lswitch,mod2.lswitch,mod3.lswitch)
...
# type = response leads to results being presented in original scale

emms.correct <- emmeans(mod1d.correct, list(pairwise ~ Group), adjust = "tukey",type = "response")
correct.emm <- emmeans(modtwo.correct,~ volat * Cond * Group)

# Group Cond    emmean     SE df lower.CL upper.CL
# HC    control  0.731 0.0174 54    0.696    0.765
# AUD   control  0.687 0.0181 54    0.650    0.723
# HC    stress   0.751 0.0174 54    0.717    0.786
# AUD   stress   0.688 0.0181 54    0.652    0.724

contrast(correct.emm,"consec", simple ="each", combine = TRUE, adjust = "mvt")

# Cond    Group contrast         estimate      SE    df t.ratio p.value
# control .     AUD - HC         -0.04395 0.02506    54 -1.754  0.2528 
# stress  .     AUD - HC         -0.06323 0.02506    54 -2.523  0.0494 
# .       HC    stress - control  0.02091 0.00907 17860  2.304  0.0717 
# .       AUD   stress - control  0.00162 0.00940 17860  0.172  0.9977 

#Results are averaged over some or all of the levels of: volat 
#Degrees-of-freedom method: containment 
#P value adjustment: mvt method for 4 tests 

# compare levels of volat within each level of stress condition
emms.winstay <- emmeans(mod1a.w_stay, list(pairwise ~ volat|Cond), adjust = "tukey",type = "response")

...
# move important variables to first columns
data_strials <- data_strials %>%select(winstayanova, everything())
data_strials <- data_strials %>%select(loseshiftanova, everything())
data_strials <- data_strials %>%select(volat, everything())
data_strials <- data_strials %>%select(choiceanova, everything())

# use single trial dataset to predict Correct, w_stay, l_switch
...
mod2.l_switch <-glmer(l_switch~Group*Cond*volat+(Cond*volat|sub_idx), data=data_new,family=binomial)

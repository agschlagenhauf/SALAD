function [loglik, traj] = model_RL_iDU2al_2betas_withStress_scaling_allparams(parameters,subj)
...
% betas
nd_beta_win  = parameters(3);
beta_win     = exp(nd_beta_win);  

nd_beta_loss  = parameters(4);
beta_loss     = exp(nd_beta_loss); 
beta = [beta_loss, beta_win];

% determine alpha_stress based on alpha_control
scaling_alpha1_stress  = parameters(5);         % normally-distributed alpha
alpha1_stress     = sgm(nd_alpha1+scaling_alpha1_stress,1); % alpha1 (transformed to be between zero and one)
% scaling_alpha2_stress  = parameters(7);         % normally-distributed alpha
% alpha2_stress     = sgm(nd_alpha2+scaling_alpha2_stress,1); % alpha1 (transformed to be between zero and one)
% nd_alpha1_stress  = parameters(6);         % normally-distributed alpha
% alpha1_stress     = 1/(1+exp(-nd_alpha1_stress)); % alpha1 (transformed to be between zero and one)
% nd_alpha2_stress  = parameters(7);         % normally-distributed alpha
% alpha2_stress     = 1/(1+exp(-nd_alpha2_stress)); % alpha2 (transformed to be between zero and one)

alpha_stress      = [alpha1_stress,alpha1_stress];


% determine kappa_stress
scaling_kappa  = parameters(6);
kappa_stress     =sgm(nd_kappa+scaling_kappa,1);
% nd_kappa_stress  = parameters(8);
% kappa_stress     = 1/(1+exp(-nd_kappa_stress));


% determine beta_stress
scaling_beta_win  = parameters(7);
beta_win_stress   = exp(nd_beta_win+scaling_beta_win);  

scaling_beta_loss  = parameters(8);
beta_loss_stress   = exp(nd_beta_loss+scaling_beta_loss);  
% % nd_beta_win_stress  = parameters(9);
% beta_win_stress     = exp(nd_beta_win_stress);  
% 
% nd_beta_loss_stress  = parameters(10);
% beta_loss_stress     = exp(nd_beta_loss_stress); 

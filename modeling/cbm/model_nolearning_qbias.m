function [loglik] = model_nolearning_qbias(parameters,subj)

% beta
nd_qbias          = parameters(1);
qbias             = 1/(1+exp(-nd_qbias)); 
nd_beta_win       = parameters(2);
% beta_win          = 3*(1/(1+exp(-nd_beta_win))); 
beta_win          = exp(nd_beta_win);
nd_beta_loss      = parameters(3);
% beta_loss         = 3*(1/(1+exp(-nd_beta_loss))); 
beta_loss          = exp(nd_beta_loss);

beta = [beta_win, beta_win]; 

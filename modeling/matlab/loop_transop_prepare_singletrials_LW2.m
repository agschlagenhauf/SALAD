...
winstay   = NaN(ntrials, length(data));
loseshift = NaN(ntrials, length(data));
trial_idx = 1:ntrials;
...
for sub = 1:length(data)
    ...
    %if i > h
    win                  = [resp(i), R(i)];
    winstay(i, sub)   = prod(win);
    
    loss                 = [-resp(i), 1-R(i)];
    loseshift(i, sub)   = prod(loss);
    %end
    %end
    end
...
table(substart:subend,:) = [sub_idx', sub_group', trial_idx', outcome(:,sub), response(:,sub), correct(:,sub), winstay(:,sub), loseshift(:,sub)];
...
var_names =['sub_idx', 'Group', 'Trial_idx', 'Outcome', 'Choice_t','Correct', 'win_st','lose_sh'];
...
%writetable(singletrialstable,'SALAD_ST_TransOp_singletrials.csv','WriteRowNames',true, 'Delimiter', 'comma') 

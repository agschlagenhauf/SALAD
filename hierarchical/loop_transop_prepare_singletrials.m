...
%%%%%%%%%%%%
data_path = '/Users/larawieland/Documents/ECN/Promotion/SALAD/data_raw/sample1_operant/CT/'; 
...
for sub = 1:length(data)
    ...
    load([data_path data(sub).name], 'A', 'R');
...
table(substart:subend,:) = [sub_idx', sub_group', trial_idx', outcome(:,sub), response(:,sub), winstay(:,:,sub), loseshift(:,:,sub)];
...
var_names =['sub_idx', 'Group', 'Trial_idx', 'Outcome', 'Choice_t', n_win_st, n_lose_sh];
...
writetable(singletrialstable,'SALAD_CT_TransOp_singletrials.csv','WriteRowNames',true, 'Delimiter', 'comma') 

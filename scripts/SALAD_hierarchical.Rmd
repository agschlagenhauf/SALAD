...
2) imports data (data_new for full  sample, data_new_HC for only HC and excluding 1B057 who has an AUC Stress missing value)
3) sets up contrasts for phase and condition
4) sets up lmes for pcorrect, winstay, loseswitch, RT:
(a) for HC p_correct
(b) with plots
(c) for HC p_wstay, p_lswitch and RT
(d) both samples together (+ AUD)
5) conducts post-hoc tests on some of the winning models on p_correct in HC 
...
## 3) Single trial analyses: Set contrasts
```{r}
...
## 4) (a) Single trial analyses: Model comparison for p_correct with both condition and individual cortisol level
```{r, include=FALSE}
...
mod1.HC.correct <- glmer(Correct~(1|sub_idx)+aucg_scaled+volat, data=data_new_HC,family=binomial)
mod2.HC.correct <- glmer(Correct~(1|sub_idx)+aucg_scaled*volat, data=data_new_HC,family=binomial)
mod3.HC.correct <-glmer(Correct~(1|sub_idx)+(aucg_scaled|sub_idx)+volat, data=data_new_HC,family=binomial)
mod4.HC.correct <-glmer(Correct~(1|sub_idx)+aucg_scaled+(volat|sub_idx), data=data_new_HC,family=binomial)
mod5.HC.correct <-glmer(Correct~(1|sub_idx)+(aucg_scaled|sub_idx)+(volat|sub_idx), data=data_new_HC,family=binomial)
mod6.HC.correct <-glmer(Correct~(1|sub_idx)+(aucg_scaled|sub_idx)*(volat|sub_idx), data=data_new_HC,family=binomial)

anova(mod0.HC.correct, mod1.HC.correct,mod2.HC.correct,mod3.HC.correct, mod4.HC.correct, mod5.HC.correct, mod6.HC.correct)
# 
...
mod1a.HC.cond.correct <- glmer(Correct~(1|sub_idx)+Cond+num_backward, data=data_new_HC,family=binomial)
mod1.HC.cond.correct <- glmer(Correct~(1|sub_idx)+Cond+volat, data=data_new_HC,family=binomial)
mod2.HC.cond.correct <- glmer(Correct~(1|sub_idx)+Cond*volat, data=data_new_HC,family=binomial)
mod3.HC.cond.correct <-glmer(Correct~(1|sub_idx)+(Cond|sub_idx)+Cond+volat, data=data_new_HC,family=binomial)
mod4.HC.cond.correct <-glmer(Correct~(1|sub_idx)+Cond+(volat|sub_idx)+volat, data=data_new_HC,family=binomial)
mod5.HC.cond.correct <-glmer(Correct~(1|sub_idx)+(Cond|sub_idx)+(volat|sub_idx)+Cond+volat, data=data_new_HC,family=binomial)
mod6.HC.cond.correct <-glmer(Correct~(1|sub_idx)+(Cond|sub_idx)*(volat|sub_idx)+Cond*volat, data=data_new_HC,family=binomial)

anova(mod0.HC.cond.correct, mod1.HC.cond.correct,mod2.HC.cond.correct,mod3.HC.cond.correct,mod4.HC.cond.correct,mod5.HC.cond.correct,mod6.HC.cond.correct)
...
mod0b.HC.both.correct <- glmer(Correct~(1|sub_idx)+Cond+num_backward, data=data_new_HC,family=binomial)
...
# 1B057 is lacking T1_cort_ST and T1_amyl_ST (for this person extra calculation of AUC? T2 instead of T1, systematic bias? otherwise exclude) - Florian; decided on leaving in and imputation

```

## 4) (c) Single trial analyses: Plots for p_correct with both condition and individual cortisol level
```{r}
...
plot_model(mod1.HC.correct,show.values=TRUE, show.p=TRUE, title = "Odd's Ratio of winning fixed-effect model", axis.labels = c("Stable Phase","Reversal Phase","Cortisol Effect")) 
...
OR_cond <- plot_model(mod1.HC.cond.correct,show.values=TRUE, show.p=TRUE, title = "Winning mixed-effects model", axis.labels = c("Stable Phase","Reversal Phase","Condition Effect"))
...
OR_cort <- plot_model(mod1.HC.correct,show.values=TRUE, show.p=TRUE, title = "Winning mixed-effects model", axis.labels = c("Stable Phase","Reversal Phase","Cortisol Effect"))
...
## 4) (c) Single trial analyses: Plots for p_winstay, p_lswitch, RT with both condition and individual cortisol level
```{r, include=FALSE}
...
sjPlot::tab_model(mod1.HC.wstay, 
                  show.intercept = TRUE, show.est = TRUE, show.se = TRUE, show.stat = TRUE, show.df = TRUE,
                  pred.labels =c("Intercept", "Condition", "Reversal Phase", "Stable Phase"))

summary(mod1.HC.wstay)$coefficients
...
anova(mod0.HC.lswitch,mod1.HC.lswitch,mod2.HC.lswitch)

sjPlot::tab_model(mod1.HC.lswitch, 
                  show.intercept = TRUE, show.est = TRUE, show.se = TRUE, show.stat = TRUE, show.df = TRUE,
                  pred.labels =c("Intercept", "Condition", "Reversal Phase", "Stable Phase"))

summary(mod1.HC.lswitch)$coefficients
...
## 4) (d) Single trial analyses: HC+AUD for p_correct, p_winstay, p_lswitch
```{r}
contrasts(data_new$Group) <- c(0.5,-0.5)
contrasts(data_new$Cond) <- c(0.5,-0.5)
contrasts(data_new$volat) <- cbind(c1 = c(1/3,-2/3,1/3), c2 = c(1/3,1/3,-2/3))
...
## Post hoc tests for HC and p_correct
```{r}

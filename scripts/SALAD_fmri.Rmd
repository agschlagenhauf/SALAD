...
load('/cloud/project/dataframes/dat.win.rda')
...
## the following two correlation analyses were included as exploratory analyses

pcor_wm_plot <- ggscatter(data_all, x = "delta_STCT", y = "num_backward",
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "Delta ST-CT", ylab = "WM Capacity")
pcor_wm_plot

cor.test(data_all$delta_STCT, data_all$num_backward, 
                    method = "pearson")

pcor_pss_plot <- ggscatter(data_all, x = "delta_STCT", y = "PSS_Total",
          add = "reg.line", conf.int = TRUE, 
          cor.coef = TRUE, cor.method = "pearson",
          xlab = "Delta ST-CT", ylab = "Past stress exposure")
pcor_pss_plot

cor.test(data_all$delta_STCT, data_all$PSS_Total, 
                    method = "pearson")
...
longdat.ins <- melt(widedat.data.ins,
                        ...
                        id.vars=c("sub_id", "group","order","age","school_yrs","num_backward","PSS_Total"),
                        # The source columns
...
barplot.pcorrect <- ggbarplot(longdat.data_all, y = "p_correct", x = "cond",
          add = c("mean_se", "point"),
          color = "cond", width=0.4, fill = c("#00BFC4","#F8766D"), alpha = 0.5,
          position = position_dodge(0.8)) +
   geom_line(aes(x=cond, group=sub_id), color = 'darkgray',size=0.2) +
...
barplot.pcorrect_pre <- ggbarplot(longdat.data_all, y = "p_correct", x = "cond",
          add = c("mean_se", "point"),
          color = "cond", width=0.4, fill = c("#00BFC4","#F8766D"), alpha = 0.5,
          position = position_dodge(0.8)) +
   geom_line(aes(x=cond, group=sub_id), color = 'darkgray',size=0.2) +
...
barplot.pcorrect_rev <- ggbarplot(longdat.rev.data_all, y = "p_correct_rev", x = "cond",
          add = c("mean_se", "point"),
          color = "cond", width=0.4, fill = c("#00BFC4","#F8766D"), alpha = 0.5,
          position = position_dodge(0.8)) +
   xlab("Condition") + ylab("Correct responses") +
...
barplot.pcorrect_post <- ggbarplot(longdat.post.data_all, y = "p_correct_post", x = "cond",
          add = c("mean_se", "point"),
          color = "cond", width=0.4, fill = c("#00BFC4","#F8766D"), alpha = 0.5,
...
f3_results <- ggarrange(ggarrange(barplot.pcorrect + rremove("ylab"), boxplot.delta, ncol = 2, labels = c("3a", "3b")),                                              # First row with scatter plot
          ggarrange(barplot.pcorrect_pre + rremove("ylab"), 
          barplot.pcorrect_rev + rremove("ylab") ,
          barplot.pcorrect_post + rremove("ylab"), 
          ncol = 3, labels = c("3c", "3d", "3e")), # Second row with box and dot plots
...
ggsave(f3_results, filename = "f3_results.png","jpg", "/cloud/project/plots/manuscript", width = 10, height = 7)


```

## 9) Figure 6: Individual Betas in ST>CT win effect
```{r}

dat.win$Condition <- factor(dat.win$Condition, labels=c("Control", "Stress"))


# Function for plotting
plotEstimates<-function(dat.win){ggplot(dat.win,aes(x=Condition, fill = c("#F8766D","#00BFC4"), col = c("#F8766D","#00BFC4")))+ geom_boxplot(aes(lower=mean-sd,upper=mean+sd,middle=mean,ymin=mean-3*sd,ymax=mean+3*sd),stat="identity")+ theme_minimal(base_size = 20) +xlab("Condition")+ylab("Contrast estimate")+scale_fill_manual(values=c("#F8766D","#00BFC4"))
}
#enter means and sd of contrast
maineff_win <- data.frame(mean=c(-0.002396429,4.342732143),sd=c(3.442329453,4.220692753 ),Condition=as.factor(c("Control","Stress")))

dat <- ggplot_build(p)$data[[1]]

#plot
p<-plotEstimates(maineff_win)
...
p_black <- p + theme(
  ...
  axis.line = element_line(colour = "grey"),# adding a white line for x and y axis,
    axis.text = element_text(colour = "grey"),
  axis.title.x = element_text(colour = "grey"),
  axis.title.y = element_text(colour = "grey")) + geom_segment(data=dat, aes(x=xmin, xend=xmax, 
                               y=middle, yend=middle), colour="black", size=1)


#safe as png
ggsave(p_black, filename = "maineff_win.png","png", "/cloud/project/plots/manuscript")

#add label with coordinates
#p+labs(y="Contrast Estimate\n[10, 12, -2]")

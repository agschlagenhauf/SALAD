...
plotEstimates<-function(dat.win){ggplot(dat.win,aes(x=cond,fill=cond))+ geom_boxplot(aes(lower=mean-sd,upper=mean+sd,middle=mean,ymin=mean-3*sd,ymax=mean+3*sd),stat="identity")+ theme_minimal(base_size = 20)+theme(panel.background = element_rect(fill = "transparent", colour = NA),  plot.background = element_rect(fill = "transparent", colour = NA)) +xlab("Condition")+ylab("Contrast estimate")+scale_fill_manual(values=c("#00BFC4","#F8766D"))+theme(
    ...
    axis.line = element_line(colour = "white"),# adding a white line for x and y axis
    # axis.title.x = element_text(colour = "white"),
    # axis.title.x = element_text(colour = "white"),
)}
#enter means and sd of contrast
maineff_win <- data.frame(mean=c(-0.002396429,4.342732143),sd=c(3.442329453,4.220692753 ),cond=as.factor(c("Control","Stress")))
...
#safe as png
ggsave(p, filename = "maineff_win.png","png", "/cloud/project/plots/manuscript")

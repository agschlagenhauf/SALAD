...
mod1.HC.correct <- glmer(Correct~aucg_scaled+Cond+volat+(1|sub_idx), data=data_new_HC,family=binomial)
mod2.HC.correct <- glmer(Correct~aucg_scaled*Cond*volat+(1|sub_idx), data=data_new_HC,family=binomial)
...
# needed in order to get p values on lmer output
library(lmerTest)

# logarithmize RT 
data_new_HC <- data_new_HC %>% rowwise() %>% mutate(RTlog = log10(RT*1000))

# interaction between stable phase contrast (1 vs. 3) and condition
mod0.HC.RT <- lmer(RTlog~(1|sub_idx), data=data_new_HC)
mod0a.HC.RT <- lmer(RTlog~(1|sub_idx), data=data_new_HC)
mod1.HC.RT <- lmer(RTlog~Cond+volat+(1|sub_idx), data=data_new_HC)
mod2.HC.RT <- lmer(RTlog~Cond*volat+(1|sub_idx), data=data_new_HC)
...
sjPlot::plot_model(mod2.HC.RT,show.values=TRUE, show.p=TRUE, auto.label = FALSE)

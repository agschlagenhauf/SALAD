...
np_params <- rcorr(as.matrix(neuropsych_params))
...
res.condbeta <- ezANOVA(longdat.np_winloss_2,betas, wid = sub_id, within = .(cond,winloss), detailed = TRUE, type = 3)

anova_out(res.condbeta)

ezPlot(
   data = longdat.np_params
   , dv = .(betas)
   , wid = .(sub_id)
   , within = .(cond)
   , between = .(STlearning)

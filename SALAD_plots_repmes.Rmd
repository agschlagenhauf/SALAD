...
x_tick_means <- c(-0.2, 1.2)


longdat.cond.HC.correct$cond3 <- jitter(longdat.cond.HC.correct$cond2, amount=.09)


f5_pcorrect_cond <- ggplot(data = longdat.cond.HC.correct, aes(y = p_correct)) +
  
   #Add geom_() objects
   geom_point(data = longdat.cond.HC.correct %>% filter(cond=="Control"), aes(x=cond3), color = '#00BFC4', size = 1.5, 
              alpha = .6) +
   geom_point(data = longdat.cond.HC.correct %>% filter(cond=="Stress"), aes(x=cond3), color = '#F8766D', size = 1.5, 
              alpha = .6) +
   geom_line(aes(x = cond3, group = sub_id), color = 'lightgray', alpha = .3) +
  
   # geom_half_boxplot(
   #   data = longdat.HC.correct %>% filter(cond2=="1"), aes(x=cond2, y = p_correct), position = position_nudge(x = 0.3),
   #   side = "r",outlier.shape = NA, center = TRUE, errorbar.draw = FALSE, width = .2,
   #   fill = '#00BFC4') +
   # 
   # geom_half_boxplot(
   #   data = longdat.HC.correct %>% filter(cond2=="0"), aes(x=cond2, y = p_correct), position = position_nudge(x = -0.3),
   #   side = "r",outlier.shape = NA, center = TRUE, errorbar.draw = FALSE, width = .2,
   #   fill = '#F8766D') +

   geom_half_violin(
     data = longdat.cond.HC.correct %>% filter(cond2=="1"),aes(x = cond2, y = p_correct), 
     position = position_nudge(x = 0.5), alpha = .6, side = "r", fill = '#00BFC4') +

   geom_half_violin(
     data = longdat.cond.HC.correct %>% filter(cond2=="0"),aes(x = cond2, y = p_correct), 
     position = position_nudge(x = 1.5), alpha = .6, side = "r", fill = "#F8766D") +

   geom_point(data = longdat.cond.HC.correct %>% filter(cond=="Stress"), aes(x = cond2, y = score_mean[2]),
      position = position_nudge(x = 0.2), color = '#F8766D', alpha = .6, size = 2) +
   
   geom_point(data = longdat.cond.HC.correct %>% filter(cond=="Control"), aes(x = cond2, y = score_mean[1]),
   position = position_nudge(x = -0.2), color = '#00BFC4', alpha = .6, size = 2) +
   
   geom_errorbar(data = longdat.cond.HC.correct %>% filter(cond=="Stress"), aes(x = cond2, y = score_mean[2],
      ymin = score_mean[2]-ci[2],
      ymax = score_mean[2]+ci[2]), 
      position = position_nudge(0.2), color = '#F8766D', width = 0.05, size = 0.4, alpha = .5) +  
   
   geom_errorbar(data = longdat.cond.HC.correct %>% filter(cond=="Control"), aes(x = cond2, y = score_mean[1],
      ymin = score_mean[1]-ci[1],
      ymax = score_mean[1]+ci[1]), 
      position = position_nudge(-0.2), color = '#00BFC4', width = 0.05, size = 0.4, alpha = .5) +

   # geom_line(data = x_cond, aes(x = x2_tick_means, y = ), color = 'black', size = 1) +
   ...
   scale_x_continuous(breaks=c(0,1), labels=c("Control","Stress"), limits=c(-0.5,2)) +
   scale_y_continuous(limits=c(0.5,1)) +
   xlab("Condition") + ylab("Percentage of correct responses") +
   labs(title = 'Correct responses in raw data', caption='Datapoints are jittered') + 
    theme(
...
ggsave(f5_pcorrect_cond, filename = "f5_pcorrect_cond.png","jpg", "/cloud/project/plots/manuscript_plots")

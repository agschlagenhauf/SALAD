...
f3_lswitch_HC <- ggplot(data=longdat.HC.lswitch, aes(y=p_lswitch)) +
   ...
   #Add geom_() objects
   geom_point(data = longdat.HC.lswitch %>% filter(cond2=="1"), aes(x=jvolat2), color = 'dodgerblue', size = 1.5, 
              alpha = .6) +
      geom_point(data = longdat.HC.lswitch %>% filter(cond2=="0"), aes(x=jvolat2), color = 'darkorange', size = 1.5,
              alpha = .6) +
...
f5_pcorrect_cond <- ggplot(data = longdat.correct, aes(y = p_correct)) +
   ...
   #Add geom_() objects
   geom_point(data = longdat.correct %>% filter(cond2=="1"), aes(x=cond2), color = 'dodgerblue', size = 1.5, 
              alpha = .6) +
   geom_point(data = longdat.correct %>% filter(cond2=="0"), aes(x=cond2), color = 'darkorange', size = 1.5, 
              alpha = .6) +
   ...
   xlab("Condition") + ylab("Percentage of correct responses") +
   labs(title = 'No main effect of stress condition', 
           subtitle = 'ST vs. CT raw data and distributions',
           caption = 'Datapoints are jittered') + 
    theme(
